    ...
    const csv =
      ...
      Array.from(elements)
        .map((el) => {
          ...
          // Find the associated indicator element
          const indicator = el.nextElementSibling;
          if (!indicator?.classList.contains('ac-indicator')) {
            return null;
          }
          ...
          return [
            ...
            indicator.getAttribute('data-status'), // Validation status
            indicator.getAttribute('data-details')?.replace(/,/g, ';') ||
              messages.csv.noDetails, // Data-details content (commas replaced)
